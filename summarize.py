...
def process_message(gmail_message_id, sender, subject, text):
    ...
    print(f"Buffer: {summarize_res_by_message_id}")
    with open("db.json", "w") as f:
        json.dump(summarize_res_by_message_id, f)
    ...
    if priority < 3:
        ...
        Mes_tg_out.format_message("", text, action_list)
    else:
...
def respond_rest():
    ...
    Mes_tg_out.format_message("", digest, [])
...
def react(gmail_message_id, reaction):
    ...
    if reaction == IGNORE_MAIL:
        ...
        GmailOpen_GmailReply.open_gmail_message(gmail_message_id)

    elif reaction == REPLY_MAIL:
        # generate draft of the reply to gmail message with gmail_message_id using GMail Toolkit
        boo = data[gmail_message_id]
        GmailOpen_GmailReply.send_message(gmail_message_id, boo[RESPONSE])

    summarize_res_by_message_id.clear()
...
John Watson.
    ...
    GmailOpen_GmailReply.send_message('18816ebc602900f5', res[RESPONSE])
